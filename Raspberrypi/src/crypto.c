/**
  ******************************************************************************
  * @file    crypto.c
  * @brief   树莓派侧 AES-128-CTR 加解密（纯 C 软件实现）
  *
  * 直接移植自 STM32 的 aes128.c，与 openssl 实现功能等价。
  * 固定 key/IV，无状态，可随时调用。
  ******************************************************************************
  */
#include "crypto.h"
...
/* ---- key / IV 常量（与 STM32 aes128.c 一致，导出供一致性测试比对） ---- */
const uint8_t g_key[16] = {
    0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF,
    0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10
};

const uint8_t g_iv[16] = {
    0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88,
    0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x00
};

/* ================================================================
 *  AES-128 内部：S-box / 轮常量 / 工具
 * ================================================================ */
...
int aes128_ctr_crypt(const uint8_t *in, uint8_t *out, size_t len)
{
    ...
    key_expansion(g_key, round_keys);

    memcpy(counter, g_iv, 16);
