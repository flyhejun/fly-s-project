...
static void SOFT_I2C_Delay(void)
{
    ...
    uint32_t cycles = SOFT_I2C_DELAY_US * (SystemCoreClock / 1000000U);
...
void SOFT_I2C_WriteByteTo(uint8_t dev_addr, uint8_t reg_addr, uint8_t data)
{
    ...
    SOFT_I2C_SendByte(dev_addr << 1);        // 设备地址 + 写位(bit0=0)
    SOFT_I2C_WaitAck();

    SOFT_I2C_SendByte(reg_addr);              // 寄存器地址
    SOFT_I2C_WaitAck();

    SOFT_I2C_SendByte(data);                  // 数据
    SOFT_I2C_WaitAck();
...
uint8_t SOFT_I2C_ReadBytesFrom(uint8_t dev_addr, uint8_t reg_addr, uint8_t *buf, uint8_t len)
{
    ...
    SOFT_I2C_SendByte(dev_addr << 1);        // 设备地址 + 写位
    if (SOFT_I2C_WaitAck() != 0)
    {
        return 1;
    }

    SOFT_I2C_SendByte(reg_addr);              // 起始寄存器地址
    if (SOFT_I2C_WaitAck() != 0)
    {
        return 1;
    }

    SOFT_I2C_Start();                          // 重复开始信号

    SOFT_I2C_SendByte((dev_addr << 1) | 1);   // 设备地址 + 读位(bit0=1)
    if (SOFT_I2C_WaitAck() != 0)
    {
        return 1;
    }
    ...
    for (i = 0; i < len; i++)
    {
         ...
         */
        buf[i] = SOFT_I2C_ReadByte(i < (len - 1) ? 1 : 0);
    }
